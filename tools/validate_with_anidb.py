...
Usage:
    python tools/validate_with_anidb.py --input data/training/nyaa_titles_5000_raw.txt
    python tools/validate_with_anidb.py --benchmark
"""
...
def call_zantetsu_parser(filename: str, mode: str = "heuristic") -> dict:
    ...
    try:
        result = subprocess.run(
            ["target/release/zantetsu-parse", mode],
            input=filename,
            capture_output=True,
            text=True,
            timeout=5,
...
def search_anime(title: str, source: str = "both") -> Optional[dict]:
    ...
    try:
        encoded_title = urllib.parse.quote(title[:200])  # API limit
        url = (
            f"{ANIMEDB_BASE}/search/realtime?q={encoded_title}&source={source}&limit=5"
        )

        req = urllib.request.Request(
            url, headers={"User-Agent": "ZantetsuValidator/1.0"}
        )
...
def validate_filename(filename: str, expected: Optional[dict] = None) -> dict:
    ...
    # Step 6: Validate other fields against API result
    if expected:
        for field in ["group", "episode", "resolution", "video_codec", "source"]:
            if field in expected:
                parsed_field = parsed.get(field)
...
def run_benchmark() -> dict:
    ...
    benchmark_file = Path("data/training/silver_dataset.jsonl")
