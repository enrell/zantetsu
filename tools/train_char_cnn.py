...
class CharCnnCrf(nn.Module):
    ...
    def forward(
        ...
        if tags is not None and mask is not None and self.use_crf:
...
def viterbi_decode(
    ...
    return [best_tags[i] for i in range(batch_size)]
...
class AugmentedDataset(Dataset):
    """Dataset with RAD augmentations applied on-the-fly."""
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        char_indices, tags = self.base_dataset[idx]
...
def evaluate(
    ...
    accuracy = compute_accuracy(all_preds, all_labels, mask.tolist())
...
def export_to_onnx(model: nn.Module, output_path: str):
    ...
    torch.onnx.export(
        ...
        dynamic_axes={
            "char_indices": {0: "batch_size", 1: "seq_len"},
...
def main():
    ...
    train_dataset = AugmentedDataset(base_dataset, augmenter, aug_ratio=0.4)
    ...
    for epoch in range(args.epochs):
        train_loss = train_epoch(
            model, train_loader, optimizer, scheduler, device, use_crf=not args.no_crf
        )
        ...
        if train_loss < best_loss:
            ...
            torch.save(model.state_dict(), "models/best_char_cnn_crf.pt")
    ...
    model.load_state_dict(torch.load("models/best_char_cnn_crf.pt"))
